...
def test_view(request):
    return render(request, 'test_render_by_server.html')

def social_success(request):
    user = request.user
    user_nickname = user.userprofile.nickname 
    response = HttpResponseRedirect('/')
    #response.set_cookie(key='nickname', value=user.userprofile.nickname)
    return response 
...
class UserInfoTestView(APIView): 
    ...
    def get(self, request):
        if not request.user.is_authenticated():
            return Response({"auth":"使用者尚未登入"}, status=200)
...
class LogoutView(APIView):
    ...
    def __logout(self, request):
        ...
        # TODO 要換成 RESTful API 寫法，response 200
...
class GeneralSignUpView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response({"error":"Email格式錯誤"},
            status=status.HTTP_400_BAD_REQUEST)
        
        # password confirm validation
        if password != confirm_password:
            return Response({"error":"密碼輸入不一致"},
            status=status.HTTP_400_BAD_REQUEST)
        ...
        pre_at_id = user.username.split("@")[0]
        profile.nickname = pre_at_id[:30]
...
class FindPasswordView(APIView):
    ...
    def __send_reset_password_url_email_to(self, user):
        ...
        email_content = (
                ...
                reset_password_url="http://127.0.0.1:8000/accounts/reset_password/" + rt.url_token,
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response({"error": "沒有此Email帳戶"},
            status=status.HTTP_403_FORBIDDEN)
        else:
        # 因為 Oauth 帳戶沒有密碼，所以不提供這功能
            if user.social_auth.exists():
                return Response({"error":"Oauth使用者不能使用此功能"},
                status=status.HTTP_403_FORBIDDEN)
...
class ResetPasswordView(APIView):
    ...
    def __check_validity_of_reset_token(self, reset_token):
        # check validity of reset password token
        if reset_token.is_valid():
            return None
        else:
            return Response({"error": "驗證連結已失效"},
            status=status.HTTP_403_FORBIDDEN)
    ...
    def get(self, request, url_token):
        ...
        try:
            user_reset_password_token = ResetPasswordToken.objects.get(url_token=url_token)
        except ResetPasswordToken.DoesNotExist:
            return Response({"error": "無效的連結"},
            status=status.HTTP_403_FORBIDDEN)
        
        # 限定存取頁面只能夠存取一次
        # 如果存取超過一次接下來就會被判定連結失效
        user_reset_password_token.reach_count += 1
        user_reset_password_token.save()
        
        err_response = self.__check_validity_of_reset_token(user_reset_password_token)
    ...
    def post(self, request, url_token):
        ...
        # 密碼設置完成後就要讓 Reset Token 變成已經使用，因為
        # 確保這個 URL 不被重複使用
        user_reset_password_token.is_used = True
        user_reset_password_token.save()
